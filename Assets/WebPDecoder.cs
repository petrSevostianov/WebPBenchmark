...
public static class WebPDecoder {
	...
	*/
	public const int WEBP_DECODER_ABI_VERSION = 0x0205;

	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
    private static extern void WebPFree(IntPtr p);

	private static byte[] MemoryToByteArray(IntPtr memory, int size) {
		var result = new byte[size];
		Marshal.Copy(memory, result, 0, size);
		return result;
	}

	private static byte[] MemoryToByteArrayAndFree(IntPtr memory, int size) {
		var result = new byte[size];
		Marshal.Copy(memory, result, 0, size);
		WebPFree(memory);
		return result;
...
// Decodes WebP images pointed to by 'data' and returns RGBA samples, along
	...
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	private static extern IntPtr WebPDecodeRGBA(byte[] data, UIntPtr data_size, out int width, out int height);

	public static byte[]? WebPDecodeRGBA(byte[] data, out int width, out int height) {
		var memory = WebPDecodeRGBA(data, (UIntPtr)data.Length, out width, out height);
		if (memory == IntPtr.Zero) {
			width = 0;
			height = 0;
			return null;
		}
		return MemoryToByteArrayAndFree(memory, width * height * 4);
	}
	/*
	...
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	private static extern IntPtr WebPDecodeARGB(byte[] data, UIntPtr data_size, out int width, out int height);
	public static byte[]? WebPDecodeARGB(byte[] data, out int width, out int height) {
		var memory = WebPDecodeARGB(data, (UIntPtr)data.Length, out width, out height);
		if (memory == IntPtr.Zero) {
			width = 0;
			height = 0;
			return null;
...
// Same as WebPDecodeRGBA, but returning B, G, R, A, B, G, R, A... ordered data.
	...
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	private static extern IntPtr WebPDecodeBGRA(byte[] data, UIntPtr data_size, out int width, out int height);
	public static byte[]? WebPDecodeBGRA(byte[] data, out int width, out int height) {
		var memory = WebPDecodeBGRA(data, (UIntPtr)data.Length, out width, out height);
		if (memory == IntPtr.Zero) {
			width = 0;
			height = 0;
			return null;
...
// Same as WebPDecodeRGBA, but returning R, G, B, R, G, B... ordered data.
	...
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	private static extern IntPtr WebPDecodeRGB(byte[] data, UIntPtr data_size, out int width, out int height);
	public static byte[]? WebPDecodeRGB(byte[] data, out int width, out int height) {
		var memory = WebPDecodeRGB(data, (UIntPtr)data.Length, out width, out height);
		if (memory == IntPtr.Zero) {
			width = 0;
			height = 0;
			return null;
		}
		return MemoryToByteArrayAndFree(memory, width * height * 3);
	}
...
// Same as WebPDecodeRGB, but returning B, G, R, B, G, R... ordered data.
	...
	*/
	...
	WEBP_EXTERN(uint8_t*) WebPDecodeRGBAInto(const uint8_t* data, size_t data_size, uint8_t* output_buffer, size_t output_buffer_size, int output_stride);
	WEBP_EXTERN(uint8_t*) WebPDecodeARGBInto(const uint8_t* data, size_t data_size, uint8_t* output_buffer, size_t output_buffer_size, int output_stride);
	WEBP_EXTERN(uint8_t*) WebPDecodeBGRAInto(const uint8_t* data, size_t data_size, uint8_t* output_buffer, size_t output_buffer_size, int output_stride);
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	...
	WEBP_EXTERN(uint8_t*) WebPDecodeRGBInto(const uint8_t* data, size_t data_size, uint8_t* output_buffer, size_t output_buffer_size, int output_stride);
	WEBP_EXTERN(uint8_t*) WebPDecodeBGRInto(const uint8_t* data, size_t data_size, uint8_t* output_buffer, size_t output_buffer_size, int output_stride);
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	...
	*/
	[StructLayout(LayoutKind.Sequential)]
	public struct WebPYUVABuffer {              // view as YUVA
		public IntPtr y, u, v, a;     // pointer to luma, chroma U/V, alpha samples
		public int y_stride;               // luma stride
		public int u_stride, v_stride;     // chroma strides
		public int a_stride;               // alpha stride
		public UIntPtr y_size;              // luma plane size
		public UIntPtr u_size, v_size;      // chroma planes size
	...
	*/
	...
	public struct WebPDecBuffer {
		...
		public int is_external_memory;    // If true, 'internal_memory' pointer is not used.
		public WebPBufferUnion u;                       // Nameless union of buffer parameters.
		public uint pad0;     // padding for later use
		public uint pad1;     // padding for later use
		public uint pad2;     // padding for later use
		public uint pad3;     // padding for later use		

		public IntPtr private_memory;   // Internally allocated memory (only when
										// is_external_memory is false). Should not be used
										// externally, but accessed via the buffer union.
	};
	...
	*/
	...
	public static bool WebPInitDecBuffer(ref WebPDecBuffer buffer) {
		return WebPInitDecBufferInternal(ref buffer, WEBP_DECODER_ABI_VERSION) != 0;
	}
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	public static extern void WebPFreeDecBuffer(ref WebPDecBuffer buffer);

	/*public static Disposable<WebPDecBuffer> CreateDecBuffer() {
		if (!WebPInitDecBuffer(out var buffer)) {
			throw new Exception("WebPInitDecBuffer failed");
		}
		return new Disposable<WebPDecBuffer>(buffer, (WebPDecBuffer) => WebPFreeDecBuffer(ref buffer));
	}*/
	...
	*/
	public static IntPtr WebPIDecGetYUV(IntPtr idec, out int last_y, out IntPtr u, out IntPtr v, out int width, out int height, out int stride, out int uv_stride) {
		return WebPIDecGetYUVA(idec, out last_y, out u, out v, out IntPtr a, out width, out height, out stride, out uv_stride, out int a_stride);
	}
	...
	*/
	...
	public struct WebPBitstreamFeatures {
		...
		// Unused for now:
		public int no_incremental_decoding;  // if true, using incremental decoding is not
											 // recommended.
	...
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	public static extern VP8StatusCode WebPGetFeaturesInternal(byte[] data, UIntPtr data_size, out WebPBitstreamFeatures features, int version);
	public static WebPBitstreamFeatures WebPGetFeatures(byte[] data) {
		var status = WebPGetFeaturesInternal(data, (UIntPtr)data.Length, out var features, WEBP_DECODER_ABI_VERSION);
		if (status != VP8StatusCode.VP8_STATUS_OK) {
			throw new Exception($"WebPGetFeaturesInternal failed: {status}");
		}
		return features;
	}
	...
	*/
	public static bool WebPInitDecoderConfig(out WebPDecoderConfig config) {
		config = new WebPDecoderConfig();
		return WebPInitDecoderConfigInternal(ref config, WEBP_DECODER_ABI_VERSION) != 0;
	}
	...
	*/
	[DllImport(LIBWEBP, CallingConvention = CallingConvention.Cdecl)]
	public static extern IntPtr WebPIDecode(IntPtr data, UIntPtr data_size, ref WebPDecoderConfig config);
	...
	*/
	...
	public static byte[]? WebPDecode(byte[] data, bool premultiplied, out int width, out int height, byte[]? output = null) {
		//if (!isrgb)
		var mode = premultiplied ? WEBP_CSP_MODE.MODE_rgbA : WEBP_CSP_MODE.MODE_RGBA;
		if (!WebPInitDecoderConfig(out var config) || !WebPInitDecBuffer(ref config.output)) {
			width = height = 0;
			return null;
		}
		using var finalizer = new Disposable(() => {
			WebPFreeDecBuffer(ref config.output);
		});

		config.output.colorspace = mode;
		using var handle = new Disposable<GCHandle>(default, (h) => {
			if (h.IsAllocated) h.Free();
		});

		if (output != null) {
			var bufferSize = output.Length;
			var features = WebPGetFeatures(data);
			var imageSize = features.width * features.height * 4;
			if (bufferSize >= imageSize) {
				handle.Value = GCHandle.Alloc(output, GCHandleType.Pinned);
				var ptr = handle.Value.AddrOfPinnedObject();
				var stride = features.width * 4;
				config.output.is_external_memory = 1;
				config.output.u.RGBA.rgba = ptr;
				config.output.u.RGBA.size = (UIntPtr)bufferSize;
				config.output.u.RGBA.stride = stride;
			}
		}

		var status = WebPDecode(data, (UIntPtr)data.Length, ref config);
		if (status != VP8StatusCode.VP8_STATUS_OK) {
			width = height = 0;
			return new byte[0];
		}
		...
		if (config.output.is_external_memory != 0 && output != null) {
			return output;
		}
		var result = MemoryToByteArray(config.output.u.RGBA.rgba, (int)config.output.u.RGBA.size);
